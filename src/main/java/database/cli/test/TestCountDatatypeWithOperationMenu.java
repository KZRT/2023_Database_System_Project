...
class TestCountDatatypeWithOperationMenu implements TriggerState {
    ...
    protected TestCountDatatypeWithOperationMenu(TestConsoleInterface ci) {
        this.ci = ci;
    }
    ...
    public boolean nextInput(int i) {
        ...
        if (trigger) {
            System.out.println("Testing Count Datatypes With Operations");
            ...
            tempDataTypes = new ArrayList<>(ci.getDataTypes());
            tempOperations = new ArrayList<>(ci.getOperations());
            if(testApi.testCountDatatypeWithOperation(tempDataTypes, tempOperations, count)) {
